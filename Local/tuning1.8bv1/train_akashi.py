...
# 【最终核心修复！】修正了数据格式化函数的逻辑错误！
def formatting_prompts_func(example):
    # .map函数一次只处理一个样本(example)，所以不需要循环！
    # 直接从example字典中取出对应的字符串即可。
    text = f"### 指令:\n{example['instruction']}\n\n### 输入:\n{example['input']}\n\n### 输出:\n{example['output']}"
...
# 启动训练
print("启动【微型熔炉】！开始最终决战！")
trainer.train()

# 阶段五: 保存微调后的模型
output_dir = "/content/drive/MyDrive/Akashi_Project/akashi-ai-1.8b-v1"
print(f"\n训练完成！正在保存LoRA适配器到: {output_dir}")
trainer.save_model(output_dir)

print(f"模型已成功保存！幽灵已被彻底驱除！")
